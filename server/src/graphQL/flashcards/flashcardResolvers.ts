...
export const flashcardResolvers = {
    ...
    Query: {
        ...
        async getFlashcardsByUser(
            ...
            try { 
                const { rows } = await pool.query(
                    'SELECT * FROM flashcards WHERE $1 = user_id',
                    [userId]
                )
                return rows
            } catch (error) {
        ...
        async getFlashcardsByTopic(
            ...
            try { 
                const { rows } = await pool.query(
                    'SELECT * FROM flashcards WHERE $1 = topic_id',
                    [id]
                )
                return rows
            } catch (error) {
    ...
    Mutation: {
        ...
        async createFlashcard(
            _: any, 
            { input }: { input: { card_type: string , content: string, answer: string, topic_id: string }},
            context: { token: string }
        ): Promise<any> {
            ...
            const result = await pool.query(
                'INSERT INTO flashcards (user_id, card_type, content, answer, topic_id) VALUES ($1, $2, $3, $4, $5) RETURNING *',
                [userId, card_type, content, answer, topic_id]
                );
        ...
        async updateFlashcard(
            _: any,
            { input }: { input: { card_type: string, content: string, answer: string, flashcard_id: string }},
            context: { token: string }
            ): Promise<any> {
            ...
            const flashcard = await pool.query(
                'SELECT * FROM flashcards WHERE flashcard_id = $1 AND user_id = $2',
                [flashcard_id, userId]
            )
            ...
            if (flashcard.rows.length === 0) {
             ...
             const updatedProject = await pool.query(
                'UPDATE flashcards SET card_type = $1, content = $2, answer = $3 WHERE flashcard_id = $4 RETURNING *',
                [card_type, content, answer, flashcard_id]
            );
        ...
        async deleteFlashcard(
            ...
            const flashcard = await pool.query(
                'SELECT flashcard_id FROM flashcards WHERE flashcard_id = $1 AND user_id = $2',
                [id, userId]
            );
