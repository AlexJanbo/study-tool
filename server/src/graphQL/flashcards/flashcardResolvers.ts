...
export const flashcardResolvers = {
    ...
    Mutation: {
        ...
        async createFlashcard(
            _: any, 
            { input }: { input: { card_type: string , content: string, answer: string }},
            context: { token: string }
        ): Promise<any> {

            const { card_type, content, answer } = input
            ...
            const result = await pool.query(
                'INSERT INTO flashcards (user_id, card_type, content, answer) VALUES ($1, $2, $3, $4) RETURNING *',
                [userId, card_type, content, answer]
                );
